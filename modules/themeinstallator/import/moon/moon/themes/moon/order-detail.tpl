	...
	<table class="detail_step_by_step std">
		...
		{foreach from=$order_history item=state name="orderStates"}
			<tr class="{if $smarty.foreach.orderStates.first}first_item{elseif $smarty.foreach.orderStates.last}last_item{/if} {if $smarty.foreach.orderStates.index % 2}alternate_item{else}item{/if}">
				...
				<td align="right">{$state.ostate_name|escape:'htmlall':'UTF-8'}</td>
			</tr>
		{/foreach}
		</tbody>
	</table>
	...
	<table class="std">
		<thead>
			<tr>
				...
				<th class="history_total_details">{l s='Total price'}</th>
			</tr>
		</thead>
		<tfoot>
			<tr class="item">
			...
			{if !$product.deleted}
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td><label for="cb_{$product.id_order_detail|intval}">{convertPriceWithCurrency price=$product.product_price_wt currency=$currency convert=0}</label></td>
						<td style="text-align:right"><label for="cb_{$product.id_order_detail|intval}">{convertPriceWithCurrency price=$product.total_wt currency=$currency convert=0}</label></td>
					</tr>
				{/if}
			{/if}
		{/foreach}
	...
	<table class="detail_step_by_step std">
		<thead>
			<tr>
				...
				<th align="left">{l s='Message'}</th>
			</tr>
		</thead>
		<tbody>
		{foreach from=$messages item=message name="messageList"}
			<tr  id="order-detail-dialogue" class="{if $smarty.foreach.messageList.first}first_item{elseif $smarty.foreach.messageList.last}last_item{/if} {if $smarty.foreach.messageList.index % 2}alternate_item{else}item{/if}">
				<td valign="top">  
					<span>
					{if $message.ename}
					...
					<b>{$shop_name|escape:'htmlall':'UTF-8'}
					{/if}
					<br />
...
<form action="{$base_dir}order-detail.php" method="post" class="std" id="sendOrderMessage">
	<p id="p_add_message">{l s='Add a message:'}</p>
