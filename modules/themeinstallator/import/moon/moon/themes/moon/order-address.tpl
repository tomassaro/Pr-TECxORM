	...
	<div class="addresses">
		...
		<ul class="address item" id="address_delivery">
			...
			<li class="address_update">
			   <a href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}&amp;back=order.php&amp;step=1" title="{l s='Update'}">{l s='Update'}</a>
			</li>
		</ul>
		...
		<p class="address_add">
			<a class="address_add submit" href="{$base_dir_ssl}address.php?back=order.php&amp;step=1" title="{l s='Add'}">{l s='Add a new address'}</a>
       </p>
	    <div id="ordermsg">
			<p class="pcomment">{l s='If you want to leave us comment about your order, please write it below.'}</p>
			<p class="commenttextarea"><textarea cols="60" rows="3" name="message">{$oldMessage}</textarea></p>
		</div>
	</div>
